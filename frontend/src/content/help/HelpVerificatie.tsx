 ...
 * met één klik live tegen de actuele rekenkern verifiëren. De berekening
 * loopt via dezelfde backend-route als de Resultaten-pagina
 * (web: `POST /calculate`, desktop: Tauri `invoke("calculate")`).
 *
 * Vergelijklogica en toleranties (±2 W abs / ±2 % rel, identiek aan de
 * Rust-integratietests) staan in `lib/verificationCompare.ts`.
 */
...
import { VERIFICATION_PROJECTS, type VerificationProjectDef } from "./verificationData";
    ...
    <Card title={`${def.title} — ${def.software}`}>
      <div className="flex flex-col gap-4">
        ...
        <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-sm">
          ...
          <MetaItem label="Tolerantie" value="±2 W absoluut óf ±2 % relatief (ruimste geldt)" />
          <MetaItem label="Bron" value={def.sourcePath} />
        </dl>

        {/* Bekende afwijkingen */}
        {def.knownDeviations.length > 0 && (
        ...
        {run.status === "error" && (
          <div className="rounded-md border border-red-600/30 bg-red-600/10 px-4 py-3 text-sm text-red-400">
            <p className="font-medium">Verificatie mislukt: {run.message}</p>
        ...
        <ResultTable rooms={rows.rooms} building={rows.building} hasRun={comparison !== null} />
      </div>
    </Card>
  );
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm leading-relaxed text-on-surface-secondary">
        ...
        Elements. Onderstaande tabellen tonen per verificatieproject de verwachte
        Vabi-waarden (Φ_HL per vertrek + gebouwtotaal); met{" "}
        <em>Verifieer nu</em> draait dezelfde invoer door de actuele engine en
        worden de afwijkingen live berekend. Een vertrek is PASS bij{" "}
      ...
      <p className="text-xs leading-relaxed text-on-surface-muted">
        ...
        Het gebouwtotaal vergelijkt het Vabi-aansluitvermogen (Φ_HL,build,
        kwadratische sommatie conform erratum 2023) met{" "}
        <code className="font-mono">connection_capacity</code> uit de engine.
      </p>
    </div>
  );
}
