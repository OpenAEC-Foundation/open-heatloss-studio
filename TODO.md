...
5. **Ronde 6 — afronding (LAATSTE).**
   ...
   - ⬜ **6c UI (frontend, REST — handoff verse sessie).** Stack: **React 19 + Zustand + Tauri**. ISSO 53 onverwarmd/U-velden (`Isso53RoomFunctionCell.tsx:156-196`: isUnheated/unheatedFactor/infiltrationReductionZ) zijn AL compleet — geen werk. Resterend:
     - **ISSO 51 config-velden** → `pages/WarmteverliesInstellingen.tsx:220-261` (naast `warmup_time`/`has_night_setback`): nieuwbouw-flag `built_after_2015`, regeltype-selector `heating_control_type` (per_zone/zelflerend/thermostaat), `simultaneity_factor` (gebouwniveau, 0..1). Backend klaar R5+6a.
     - **Bouwfase-dropdown** → `components/projectSetup/VentilationPanel.tsx:~260` + `Isso53BuildingFields.tsx:~102`: `bouwfase` (nieuwbouw/bestaande_bouw) op `VentilationConfig`. Backend klaar R4.
     - **Rapport-weergave nieuwe BuildingSummary-velden** → `lib/reportBuilder.ts` (ISSO 51) + `lib/isso53ReportBuilder.ts` (ISSO 53): `phi_hl_build`/`phi_hl_verdeler` (K3), `infiltration_method_origin` (C1), `aggregation_method` (C2), `heating_up_simultaneity_factor` (K2). Result-types: `types/result.ts`, `types/isso53Result.ts:50-74`.
     - **Veld-rename `f_rh`→P / `accumulating_area`→A_g** (cross-cutting, via schema-gen): Rust `isso51-core/src/result.rs:160-176` + `calc/room_load.rs:355-359` + `lib.rs:719` (test) → `isso51-ifcx/src/namespace.rs:242`+`to_ifcx.rs:284` → `src-tauri/bin/gen_pdf.rs:254` → `npm run generate-types` → frontend `types/result.ts:37-38` + `lib/reportBuilder.ts:448-449` + `lib/isso53ChartData.ts:73-74`. Volgorde: backend struct eerst, dan schema-gen, dan frontend-consumptie. Veld draagt nu al P resp. A_g (alleen naam misleidend).
