...
- [ ] **Ventilatie-feedthrough verifiëren na recompute** — in een opgeslagen bestand (02-06) toonden 2 van 3 toevoer-ruimtes phiV=0 ondanks ingevulde q_v, en `totalVentilationLoss` was identiek aan een eerdere opslag → vermoedelijk stale result (niet herberekend). Bevestigen dat de vastgestelde q_v (`room.ventilation_rate` → `ventilationQvEstablished`) na een verse Berekenen correct doorwerkt voor álle toevoer-ruimtes; zo niet → feedthrough-bug.
